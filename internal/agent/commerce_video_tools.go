...
func CommerceVideoTools() []AgentTool {
	scriptUnitID := stringSchema("带货脚本 ID；无法唯一确定时先用 commerce.script.list 或 agent.ask_user。")
	...
	return []AgentTool{
		...
		readTool("commerce.script.get", "读取广告脚本", "读取指定广告脚本的当前正文和当前正文哈希。", authz.PermissionScriptRead, objectSchemaRequired(map[string]any{
			"scriptUnitId": scriptUnitID,
		}, "scriptUnitId")),
		...
		}, "scriptUnitId", "expectedRevision", "patch"), true),
		...
		}, "sourceScriptUnitId", "count", "dimension", "instruction"), true),
		...
		}, "sourceScriptUnitId", "dimension", "instruction", "variations")), authz.PermissionWorkflowRun),
		...
		readTool("commerce.video.options", "视频生成选项", "读取当前模型可执行时长、分辨率、音频和参考图输入契约。", authz.PermissionWorkflowRun, objectSchemaRequired(map[string]any{
			"scriptUnitId": scriptUnitID,
		}, "scriptUnitId")),
