...
- Media Worker final composition reuses existing tables: `timeline_json` and `final_video` are `artifacts.type` values, while the final MP4 also gets a `media_files` row linked to the `final_video` artifact.
