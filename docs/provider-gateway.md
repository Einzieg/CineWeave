...
Shot video production sends the complete episode script, current scene and shot, project manuals, assets, and reference policy as structured context to `shot_video_prompt_agent`, then sends the candidate and the same context to the independent `shot_video_prompt_review_agent`. The complete episode remains available for plot accuracy and continuity, but it is not an audio source: only the current shot's timing-assigned `shot.scriptDialogue` may define dialogue, narration, voiceover, or system audio. After review, the Script Worker deterministically appends a structured authoritative audio timeline to the approved visual and motion prompt. This prevents either Agent from selecting, rewriting, translating, or omitting audio and allows multiline Chinese dialogue to remain verbatim. Deterministic timeline validation runs once locally; only malformed model JSON is eligible for structured-output retries. The composed prompt is stored on `storyboard_shots.video_prompt` and submitted to the video provider. Both text calls stream through Provider Gateway, write separate workflow nodes and provider call logs, and are persisted in `storyboard_shots.metadata.videoPromptAgent` for traceability. Gateway performs a final deterministic length check after selecting the video model.
