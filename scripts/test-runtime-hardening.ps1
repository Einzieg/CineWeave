...
try {
  ...
  if ($ProviderVideoOnly) {
    Invoke-GoContainer -Integration -Arguments @(
      'go', 'test', '-count=1', './internal/provider',
      '-run', '^TestGatewayVideoOutputMismatchFinalizesAsyncTask$'
    )
    Write-Host 'Provider video output mismatch terminalization test passed.'
    return
  }

  if ($SourceToScriptOnly) {
    $sourceToScriptTests = '^(TestSourceToScriptRetryCreatesNewGenerationForFailedEpisodes|TestPrepareSourceToScriptAppendsSecondChapterToCurrentScriptIntegration|TestFinalizeSourceToScriptKeepsPreviousProjectScriptWhenAllEpisodesFailIntegration|TestSourceToScriptExplicitFirstChapterPublishesImmediatelyIntegration|TestSourceToScriptMixedFailureKeepsFallbackEpisodeAndActivatesCompleteVersionIntegration|TestSourceToScriptMissingFailedEpisodeCreatesUnactivatedPartialVersionIntegration|TestSourceToScriptRejectsSourceAndScriptChangesBeforePublicationIntegration|TestSourceToScriptManifestReindexesAfterChapterDeletionWithoutDroppingRetainedEpisodeIntegration)$'
    Invoke-GoContainer -Integration -Arguments @(
  ...
  if ($ProjectDeletionOnly) {
    $projectDeletionTests = '^(TestProjectDeletionAPILifecycleIntegration|TestProjectDeletionStorageBatchRecoversRetryAndExpiredLeaseIntegration|TestProjectDeletionCompletesAfterStorageRetryAndPreservesSharedObjectsIntegration|TestProjectDeletionCommitRemovesCommerceReferencesBeforeArtifactsIntegration|TestGatewayVideoCreateFencesDeletingProjectBeforeUpstreamIntegration)$'
    Invoke-GoContainer -Integration -Arguments @(
      'go', 'test', '-count=1', './internal/api', './internal/provider', './internal/workflows',
      '-run', $projectDeletionTests
    )
    Invoke-GoContainer -Arguments @(
      'go', 'test', '-count=1', './internal/workflows',
...
$integrationTests = '^(TestProjectEventLogSharesOutboxTransactionAndSupportsDurableCatchup|TestProviderRequestIdempotencyIntegration|TestProviderTextStreamV2RetryAndReplayIdentityIntegration|TestProviderVideoCreateIdempotencyIntegration|TestGatewayRoutingIntegration|TestGatewayVideoOutputMismatchFinalizesAsyncTask|TestDeleteModelHardDeletesAndAllowsRecreate|TestUpdateModelProfileBinding|TestDeleteModelRejectsActiveRuntimeWork|TestAssetBatchCreateRetryAndRevisionConflict|TestDerivedAssetBatchCommandPersistsFullWorksetAndRetry|TestDerivedAssetBatchAggregateApprovedAndReviewRequiredIsPartialIntegration|TestDerivedAssetExecutionRejectsStaleIdentityBeforeProviderIntegration|TestDerivedAssetLateProviderResultAfterGenerationSwitchIsDiscardedIntegration|TestDerivedAssetExecutionLeaseCASDoesNotStealUnexpiredLeaseIntegration|TestDerivedAssetExecutionFailureSynchronizesImmutableRequestOutcomeIntegration|TestDerivedAssetReconcilerRecoversEveryDurableStageIntegration|TestDerivedAssetCommitIsIdempotentAndLateResultCannotOverwriteRequirementIntegration|TestDerivedAssetBatchTerminalAggregateCountsNotFoundDuplicateAndSkippedIntegration|TestDerivedAssetRetryLineageKeepsOriginalFailureImmutableIntegration|TestSourceToScriptRetryCreatesNewGenerationForFailedEpisodes|TestPrepareSourceToScriptAppendsSecondChapterToCurrentScriptIntegration|TestFinalizeSourceToScriptKeepsPreviousProjectScriptWhenAllEpisodesFailIntegration|TestRuntimeOperationReconciliationIntegration|TestCreateWorkflowRunPersistsIdempotencyWithOutbox|TestWorkflowStartOutboxRecoversExpiredLeaseAndAlreadyStarted|TestRestartedNodeRotatesExecutionTokenAndFencesStaleAttempt|TestCancelledWorkflowRunRejectsLateTerminalTransition|TestCancellationReconcilerTerminalizesStuckProviderTask|TestProjectDeletionAPILifecycleIntegration|TestProjectDeletionStorageBatchRecoversRetryAndExpiredLeaseIntegration|TestProjectDeletionCompletesAfterStorageRetryAndPreservesSharedObjectsIntegration|TestProjectDeletionCommitRemovesCommerceReferencesBeforeArtifactsIntegration|TestGatewayVideoCreateFencesDeletingProjectBeforeUpstreamIntegration)$'
  Invoke-GoContainer -Integration -Arguments @(
    'go', 'test', '-count=1',
